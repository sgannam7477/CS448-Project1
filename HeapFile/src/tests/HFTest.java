...
class HFDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1() {
    ...
    if (status == OK) {
      ...
      while (!done) {
        ...
        if (status == OK && !done) {
          ...
          if (len != reclen) {
            ...
            status = FAIL;
            break;
          } else if (Minibase.BufferManager.getNumUnpinned() == Minibase.BufferManager.getNumBuffers()) {
      ...
      if (status == OK) {
        if (Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumBuffers()) {
